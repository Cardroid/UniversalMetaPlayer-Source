...
namespace UMP.Lib.Media.Loader
{
  public class LocalMediaLoader
  {
    ...
    public static async Task<GenericResult<MediaInformation>> TryLoadInfoAsync(MediaInformation info, bool fullLoad, IMessageProgress<double> progress = null)
    {
      if (progress != null)
      ...
      if (!string.IsNullOrWhiteSpace(path) && File.Exists(path))
      {
        await Task.Run(() =>
        {
          ...
          info.LoadedStatus = fullLoad ? MediaInfoLoadedStatus.Full : MediaInfoLoadedStatus.Semi;
        });
