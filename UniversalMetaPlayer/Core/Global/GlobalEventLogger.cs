...
namespace UMP.Core.Global
{
  public static class GlobalEventLogger
  {
    ...
    static GlobalEventLogger()
    {
      ...
      {
        if (IsEnabled)
          Log.Debug($"Event Handled GlobalProperty.PropertyChanged {e.PropertyName}");
      };
      MainMediaPlayer.PlayList.Field_PropertyChanged += (_, e) =>
      {
        if (IsEnabled)
          Log.Debug($"Event Handled MainMediaPlayer.PlayList.Field_PropertyChanged {e.PropertyName}");
      };
      MainMediaPlayer.PropertyChanged += (_, e) =>
      {
        if (IsEnabled)
          Log.Debug($"Event Handled MainMediaPlayer.PropertyChanged {e.PropertyName}");
